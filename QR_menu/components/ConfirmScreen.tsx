...
export default function ConfirmScreen({
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-[18px] px-6 py-10 text-center">
      <div className="flex h-[84px] w-[84px] items-center justify-center rounded-full bg-green text-[42px] text-[#08130a]">
        ✓
      </div>
      <div className="font-display text-[26px]">COMMANDE ENVOYÉE</div>
      <div className="text-sm text-muted">Numéro de commande</div>
      <div className="font-display text-[38px] text-green">#{orderNumber}</div>

      {scheduledFor && (
        <div className="rounded-full bg-panel px-4 py-1.5 text-xs text-muted">
          Retrait prévu à{" "}
      ...
      {status !== "cancelled" && (
        <div className="flex items-center gap-2">
          {STATUS_STEPS.map((s, i) => (
            <div
              key={s}
              className={`h-2 w-10 rounded-full ${i <= stepIndex ? "bg-green" : "bg-panel-2"}`}
            />
          ))}
        </div>
      )}
      ...
      {status === "completed" ? (
        ...
        <button
          onClick={onNewOrder}
          className="mt-2 rounded-full bg-panel px-8 py-3.5 text-sm text-fg"
        >
          Retour au menu
        </button>
      )}
...
function FeedbackForm({ orderId }: { orderId: string }) {
  ...
  if (sent) {
    return <div className="mt-2 text-sm text-green">Merci pour votre avis !</div>;
  }

  return (
    <div className="mt-2 flex w-full max-w-[280px] flex-col items-center gap-3">
      <div className="text-sm text-muted">Comment était votre commande ?</div>
      <div className="flex gap-1.5 text-2xl">
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} onClick={() => setRating(n)} className={n <= rating ? "text-green" : "text-panel-2"}>
            ★
          </button>
        ))}
      </div>
      <textarea
        ...
        rows={2}
        className="w-full rounded-xl border border-border bg-panel px-3.5 py-2.5 text-sm outline-none focus:border-green"
      />
      <button
        onClick={handleSubmit}
        disabled={rating === 0 || sending}
        className="rounded-full bg-green px-8 py-2.5 text-sm font-semibold text-[#08130a] disabled:opacity-50"
      >
        {sending ? "…" : "Envoyer"}
      </button>
    </div>
