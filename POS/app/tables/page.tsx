...
import { getTables, createTable, updateTable, deleteTable, closeSession } from "@/lib/api";
...
function FloorPlan() {
  ...
  useEffect(() => {
    ...
    const events = ["table:updated", "table:closed", "table:assigned", "table:deleted"];
            ...
            {t.session && (
              <div className="flex flex-col gap-2 border-t border-border pt-3 text-xs text-muted">
                ...
                <button
                  ...
                  className="h-11 w-full rounded-xl border border-border text-sm text-muted transition-colors motion-safe:active:scale-95 hover:border-green hover:text-fg"
                >
                  Clôturer
                </button>
              </div>
